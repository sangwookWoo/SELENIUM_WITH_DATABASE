...
id = int(sys.argv[1])
login_id = sys.argv[2]
# python recovery_program.py "430" "nuri007"

filePath, fileName = os.path.split(__file__)
logFolder = os.path.join(filePath , 'logs')
os.makedirs(logFolder, exist_ok = True)
logfilepath = os.path.join(logFolder, fileName.split('.')[0] + '_' + login_id + '_' + re.sub('-', '', str(datetime.date.today())) + '.log')
...
def main():
    ...
    # db 연결
    engine = create_engine('postgresql://mruser:%s@192.168.2.176:6343/mr_db' % quote('y^5hEXwpQYqXM%A3'))
    ...
    url = 'http://192.168.2.179:8080/login'
